...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Avaliação do RAG usando RAGAS (Sem Ground Truth / Resposta de Referência)")
    parser.add_argument("--limit", type=int, default=None,
                        help="Limite de amostras a avaliar para testes rápidos (ex: --limit 3)")
    parser.add_argument("--dataset", type=str, default=default_dataset,
                        help="Caminho para o arquivo dataset de entrada JSON")
    parser.add_argument("--output", type=str, default=default_output,
                        help="Arquivo CSV de saída dos resultados detalhados")
    ...
    print(
        f"Iniciando avaliação automática com RAGAS em {len(raw_data)} amostras...")
    ...
    for idx, row in df_res.iterrows():
        print(f"\nAmostra #{idx + 1}")
        print(
            f"  Pergunta  : {row.get('user_input', row.get('question', ''))}")
        ...
        print(
            f"    - Answer Relevancy   : {row.get('answer_relevancy', 0.0):.4f}")

        ctx_rel_score = row.get('context_relevance',
                                row.get('nv_context_relevance', 0.0))
